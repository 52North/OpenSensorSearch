...
<%
	...
	if (request.getParameter("sendRequest") != null) {
		insertSensorStatus
				.setResponseString(Client
						.sendPostRequest(insertSensorStatus
								.getRequestString()));
	}
%>
...
<ul class="inputTablesList">
	...
	<table>
		<tr>
			...
			<td><input type="text" class="inputField"
				...
				value="<%=insertSensorStatus.getSensorIDInSIRValue()%>" /></td>
		</tr>
		<tr>
			<td class="inputTitle">Property Name:</td>
			<td><input type="text" class="inputField" name="propertyName"
				size="100" value="<%=insertSensorStatus.getPropertyName()%>" /></td>
		</tr>
		<tr>
			<td class="inputTitle">Property Value:</td>
			<td><input type="text" class="inputField" name="propertyValue"
				size="100" value="<%=insertSensorStatus.getPropertyValue()%>" /></td>
		</tr>
		<tr>
			<td class="inputTitle">UOM (optional):</td>
			<td><input type="text" class="inputField" name="uom" size="100"
				value="<%=insertSensorStatus.getUom()%>" /></td>
		</tr>
		<tr>
	...
	<p>You can insert multiple StatusDesription elements manually in below before sending the request.</p>
	</li>
</ul>
...
<p class="textareaBorder"><textarea id="requestStringArea"
	class="mediumTextarea" name="requestString" rows="10" cols="10"><%=insertSensorStatus.getRequestString()%></textarea></p>
<p><input type="submit" name="sendRequest" value="Send request" /></p>
</form>
<p class="textareaBorder"><textarea id="responseStringArea"
	class="mediumTextarea" rows="10" cols="10"><%=insertSensorStatus.getResponseString()%></textarea></p>
